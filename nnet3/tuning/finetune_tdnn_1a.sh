...
src_dir=Librispeech-model-mct-tdnnf/model
dir=exp/${src_dir}_${data_set}
...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc_pitch.sh \
    --pitch-config conf.bak/pitch.conf --cmd "$train_cmd" --nj $nj --mfcc-config conf.bak/mfcc.conf \
  ...
  steps/make_mfcc.sh \
    --cmd "$train_cmd" --nj $nj --mfcc-config conf.bak/mfcc_hires.conf \
